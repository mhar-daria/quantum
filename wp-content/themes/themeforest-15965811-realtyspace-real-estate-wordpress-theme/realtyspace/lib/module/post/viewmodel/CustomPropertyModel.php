...
class CustomPropertyModel
{
    ...
    public function related_properties( $properties = array(), $options = array(), $limit = 3 ) {
        ...
        if ( ! count($options) ) {
            $options['Ad_Type'] = $this->property['Ad_Type'];
        ...
        foreach ($properties as $property) {
            ...
            if ( isset($options['Ad_Type']) ) {
                if ( $property['Ad_Type'] != $options['Ad_Type'] ) {
                    continue;
                }
            }

            if ( isset($options['Community']) ) {
                if ( $property['Community'] != $options['Community'] ) {
                    continue;
                }
            }

            if ( isset($options['Bedrooms']) ) {
                if ( $property['Bedrooms'] < ($options['Bedrooms']-1) || $property['Bedrooms'] > ($options['Bedrooms']+1) ) {
                    continue;
                }
            }

            if ( isset($options['Price']) ) {
                if ( $property['Price'] < $options['Price']['min'] || $property['Price'] > $options['Price']['max'] ) {
                    continue;
                }
            }
    ...
    public function breadcrumbs_link()
    {
